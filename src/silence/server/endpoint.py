...
def setup_endpoint(
    ...
    def route_handler(*args, **kwargs):
        ...
        if sql_op == SQL.SELECT:
            ...
            body_params = {
                param: form.get(param, None) for param in request_body_params
            }
    ...
    server_manager.APP.add_url_rule(
        flaskify_url(full_route), method + route, route_handler, methods=[method]
    )
    server_manager.API_SUMMARY.register_endpoint(
        {"route": full_route, "method": method.upper(), "description": description}
    )
