...
def load_user_endpoints():
    ...
    if path.isdir(auto_dir):
        endpoint_paths_json_default = [
            auto_dir + f"/{f}" for f in listdir(auto_dir) if f.endswith(".json")
        ]
        ...
        with open(jsonfile, "r", encoding="utf8") as ep:
            ...
            for endpoint in endpoints:
                kwargs_nones = dict(
                    ...
                    request_body_params=endpoint.get("request_body_params"),
                )

                kwargs = {k: v for k, v in kwargs_nones.items() if v is not None}

                server_endpoint.setup_endpoint(
