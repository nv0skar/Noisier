...
class Settings:
    def __init__(self):
        ...
        try:
            ...
            if not hasattr(self, "SECRET_KEY"):
                ...
                print(
                    "If you have git clone'd this project, use 'silence new' instead,"
                )
            ...
            if "run" in sys.argv or "createdb" in sys.argv:
                print(
                    "Can't find the settings.py file! Are you running this command from the project's root folder?"
                )
                sys.exit(1)
    ...
    def warn_unknown_settings(self):
        ...
        if self.unknown_settings:
            logger.warning(
                "The following parameters in your settings.py are "
                "unknown and will be ignored: " + ", ".join(self.unknown_settings)
            )

    def __str__(self):
        return pprint.pformat(
            {setting: getattr(self, setting) for setting in self.setting_keys}
        )
